...
import { commentSongs, getSongDetail, likeSongs } from "../store/action/userAction"
import {
  Typography,
  Spin,
  Input,
...
const SongDetails = () => {
  ...
  useEffect(() => {
    if (songDetail) {
      ...
      const hasLiked = songDetail.likes.some((like) => like.userId === currentUserId)
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      // Success message
      messageApi.success(isLiked ? "Removed from favorites" : "Added to favorites")
    } catch (error) {
      ...
      <div className="bg-gradient-to-b from-[#1E1E2E] to-[#0F0F17] min-h-screen p-4 md:p-10 text-white">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <Skeleton.Image active className="w-full h-[400px] rounded-2xl" />
            <div className="space-y-6">
              <Skeleton active paragraph={{ rows: 1 }} title={{ width: "80%", style: { height: "40px" } }} />
              <Skeleton active paragraph={{ rows: 1 }} title={{ width: "50%" }} />
              <Skeleton.Button active size="large" shape="round" block className="mt-4" />
  ...
  if (!songDetail) {
    return (
      <div className="flex items-center justify-center h-screen bg-[#1E1E2E] text-white">
        <Spin size="large" tip="Loading song details..." />
      </div>
    )
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            <div className="w-full group relative">
              <div className="overflow-hidden rounded-2xl shadow-[0_0_30px_rgba(155,76,139,0.3)] transition-all duration-500 hover:shadow-[0_0_40px_rgba(155,76,139,0.5)]">
                <img
                  src={songDetail.coverImage || "/placeholder.svg?height=500&width=500"}
            ...
            <div className="flex flex-col h-full justify-between">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <Tag color="#9B4C8B" className="uppercase text-xs font-bold tracking-wider">
                    {songDetail.genre}
                  </Tag>
                ...
                <Title level={1} className="!text-white text-3xl md:text-4xl lg:text-5xl font-bold mb-2 leading-tight">
                  {songDetail.title}
                </Title>

                <Text className="text-gray-300 text-lg block mb-4">
                  By <span className="text-[#BB6AA6] hover:underline cursor-pointer">{songDetail.artist.username}</span>
                </Text>

                <div className="flex flex-wrap items-center gap-4 mb-6 text-sm">
                  <Tooltip title="Plays">
                    <div className="flex items-center gap-1 text-gray-300">
                      <EyeOutlined /> {songDetail.plays.toLocaleString()}
                    </div>
                  </Tooltip>

                  <Tooltip title="Likes">
                    <div className="flex items-center gap-1 text-gray-300">
                      <HeartOutlined /> {likeCount.toLocaleString()}
                    </div>
                  </Tooltip>

                  <Tooltip title="Comments">
                    <div className="flex items-center gap-1 text-gray-300">
                      <MessageOutlined /> {comments.length.toLocaleString()}
                    </div>
                  </Tooltip>

                  <Tooltip title="Duration">
                    <div className="flex items-center gap-1 text-gray-300">
                      <ClockCircleOutlined /> {songDetail.duration}s
                    </div>
                  </Tooltip>
                </div>
                ...
                <div className="bg-[#2A2331]/80 backdrop-blur-sm rounded-xl p-4 mb-6">
                  ...
                  <div className="flex items-center gap-4 mb-3">
                    ...
                    <Tooltip title="Volume">
                      <SoundOutlined className="text-xl text-gray-300 hover:text-white cursor-pointer" />
                    </Tooltip>
                  </div>
                </div>
              </div>
              ...
              {/* Song Info */}
              <Card className="bg-[#2A2331]/60 backdrop-blur-sm border-gray-700 rounded-xl">
                <div className="text-gray-300 space-y-3">
                  

                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div>
                      <Text className="text-gray-400 block">Release Date</Text>
                      <Text className="text-white">{new Date(songDetail.releaseDate).toLocaleDateString()}</Text>
                    </div>

                    <div>
                      <Text className="text-gray-400 block">Last Updated</Text>
                      <Text className="text-white">{new Date(songDetail.updatedAt).toLocaleDateString()}</Text>
                    </div>

                    <div>
                      <Text className="text-gray-400 block">Genre</Text>
                      <Text className="text-white">{songDetail.genre}</Text>
                    </div>

                    <div>
                      <Text className="text-gray-400 block">Duration</Text>
                      <Text className="text-white">{songDetail.duration}s</Text>
                    </div>
                  </div>
                </div>
              </Card>
          ...
          <div id="comments-section" className="mt-16 scroll-mt-8">
            <Divider className="border-gray-700 mb-8">
              <Title level={3} className="!text-white !m-0 flex items-center">
                ...
                Comments <span className="text-gray-400 ml-2">({comments.length})</span>
              </Title>
            </Divider>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex justify-between items-center">
                    <Text className="text-gray-400 text-xs">{commentText.length}/500 characters</Text>
                    <Button
                      type="primary"
            ...
            {comments.length > 0 ? (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between items-center mb-2">
                          <div>
                            ...
                            {comment.user?.isArtist && (
                              <Tag color="#BB6AA6" className="ml-2 text-xs">
                                Artist
                              </Tag>
                            )}
                          </div>
                        ...
                        <Paragraph className="text-gray-300 mb-2 whitespace-pre-line">{comment.text}</Paragraph>
                      </div>
                    </div>
                  </Card>
                )}
              ...
              <Card className="bg-[#2A2331]/60 backdrop-blur-sm border-gray-700 rounded-xl text-center py-8">
                <div className="flex flex-col items-center justify-center">
                  ...
                  <Title level={4} className="!text-gray-300 !m-0 mb-2">
                    No comments yet
                  </Title>
                  <Text className="text-gray-400">Be the first to share your thoughts about this track!</Text>
                </div>
              </Card>
            )}
          </div>
